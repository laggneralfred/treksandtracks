...
Plugin URI: http://ithemes.com/purchase/billboard-wordpress-ad-manager-plugin/
...
if ( ! class_exists( 'iThemesBillboard' ) ) {
	class iThemesBillboard {
		...
		function index() {
			...
			if ( isset( $_REQUEST['group_id'] ) && empty( $_REQUEST['cancel_save_group'] ) ) {
				...
				if ( ! empty( $_REQUEST['view_entries'] ) ) {
					...
					if ( isset( $_REQUEST['entry_id'] ) && empty( $_POST['cancel_save_entry'] ) ) {
		...
		function _entrySave() {
			...
			if ( empty( $url ) ) {
				$this->_errors[] = 'url';
				$this->_showErrorMessage( 'A Link URL is required' );
			}
			elseif ( ! preg_match( '[^(https?|ftp)://]i', $url ) ) {
				$this->_errors[] = 'url';
			...
			if ( $this->_errors ) {
				$this->_showErrorMessage( 'Please correct the ' . ngettext( 'error', 'errors', count( $this->_errors ) ) . ' in order to modify this entry\'s settings' );
	...
	<div class="wrap">
		...
		<form name="addnew" id="addnew" enctype="multipart/form-data" method="post" action="<?php echo $this->_selfLink; ?>">
			...
			<p class="submit">
				...
				<?php $this->_addSubmit( 'cancel_save_entry', 'Cancel' ); ?>
			</p>
...
<?php
		...
		function _entriesDelete() {
			...
			if ( $names ) {
				...
				$this->_showStatusMessage( 'Successfully deleted the following ' . ngettext( 'entry', 'entries', count( $names ) ) . ': ' . implode( ', ', $names ) );
			}
			else
		...
		function _entriesCreate() {
			...
			if ( empty( $url ) ) {
				$this->_errors[] = 'url';
				$this->_showErrorMessage( 'A Link URL is required to create a new entry' );
			}
			elseif ( ! preg_match( '[^(https?|ftp)://]i', $url ) ) {
				$this->_errors[] = 'url';
				$this->_showErrorMessage( 'The URL value must be a valid link in the form of either "http://domain.com/", "https://domain.com/", or "ftp://domain.com/"' );
...
<?php
		...
		function _groupSave() {
			...
			if ( $this->_errors )
				$this->_showErrorMessage( 'Please correct the ' . ngettext( 'error', 'errors', count( $this->_errors ) ) . ' in order to modify this Billboard Group\'s settings' );
			else {
	...
	<div class="wrap">
		...
		<form enctype="multipart/form-data" method="post" action="<?php echo $this->_selfLink; ?>">
			...
			<p class="submit">
				...
				<?php $this->_addSubmit( 'cancel_save_group', 'Cancel' ); ?>
			</p>
...
<?php
		...
		function _groupsDelete() {
			...
			if ( $names )
				$this->_showStatusMessage( 'Successfully deleted the following Billboard ' . ngettext( 'Group', 'Groups', count( $names ) ) . ': ' . implode( ', ', $names ) );
			else
		...
		function _groupsCreate() {
			...
			if ( $this->_errors )
				$this->_showErrorMessage( 'Please correct the ' . ngettext( 'error', 'errors', count( $this->_errors ) ) . ' in order to add the new Billboard Group' );
			else {
				$group = array();
...
<?php
		...
		function widgetsRender( $args, $widget_args = 1 ) {
			...
			echo '<div class="' . $this->_class . ' ' . $this->_class . "-$css_class\" id=\"$css_id\"$alignment>\n";
...
<?php
		...
		function widgetsControl( $widget_args = 1 ) {
			...
			if ( empty( $this->_options['widgets'][$number]['alignment'] ) )
				$this->_options['widgets'][$number]['alignment'] = 'center';
...
<?php
	...
	$iThemesBillboard = new iThemesBillboard();
}
